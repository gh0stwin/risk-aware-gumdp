...
from risk_aware_gumdp.gym.cost import EntropyCostFn
from risk_aware_gumdp.gym.utils import Params


@dataclass
class GridExplorationOccupancyState:
...
@dataclass
class GridExplorationOccupancyObs:
    state_index: np.ndarray
    occupancy: np.ndarray
    is_stuck: np.ndarray


class GridExplorationOccupancyEnv(FuncEnv):
    N_ACTIONS = 4
    LEFT = 0
    UP = 1
    RIGHT = 2
    ...
    def __init__(
        ...
        self.cost_fn = EntropyCostFn(norm=True)
        self.gamma = gamma
        self.h = h

    def initial(self, rng: np.random.Generator, params: Params | None = None) -> GridExplorationOccupancyState:
    ...
    def reward(
        ...
        state: GridExplorationOccupancyState,
        action: np.ndarray,
        next_state: GridExplorationOccupancyState,
        rng: np.random.Generator,
        params: Params | None = None,
    ) -> np.ndarray:
    ...
    def terminal(
        self, state: GridExplorationOccupancyState, rng: np.random.Generator, params: Params | None = None
    ) -> tuple[np.ndarray, np.ndarray]:
        return state.step_count >= self.h, np.array(False)

    def observation(
        self, state: GridExplorationOccupancyState, rng: np.random.Generator, params: Params | None = None
    ) -> GridExplorationOccupancyObs:
        norm = (1 - self.gamma) / (1 - self.gamma**self.h)
        return GridExplorationOccupancyObs(state_index=state.state_index.copy(), occupancy=state.occupancy * norm)
